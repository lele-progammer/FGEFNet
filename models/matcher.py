...
class HungarianMatcher_Crowd(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        out_prob = outputs["pred_logits"].flatten(0, 1).softmax(-1)  # 预测点为各个类别的概率 [batch_size * num_queries, num_classes]
        out_points = outputs["pred_points"].flatten(0, 1)  # 预测点的坐标 [batch_size * num_queries, 2]

        # Also concat the target labels and points
        # tgt_ids = torch.cat([v["labels"] for v in targets])
        tgt_ids = torch.cat([v["labels"] for v in targets])  # 真实点的标签 [一个batch总的真实点数, 1]
        tgt_points = torch.cat([v["point"] for v in targets]) # 真实点的坐标 [一个batch总的真实点数,2]
        ...
        C = C.view(bs, num_queries, -1).cpu()  #C [B, num_queries, 一个batch总的真实点数]
